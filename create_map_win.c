...
/*   Updated: 2024/03/28 17:00:43 by mpihur           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_keys(int keysum, void *param)
{
	...
	if (keysum == XK_w || keysum == XK_Up)
		put_images_to_win((t_win **)&param,
			check_and_change(((t_win *)param), 4, NULL, NULL), 0, 0);
	if (keysum == XK_s || keysum == XK_Down)
		put_images_to_win((t_win **)&param,
			check_and_change(((t_win *)param), 3, NULL, NULL), 0, 0);
	if (keysum == XK_a || keysum == XK_Left)
		put_images_to_win((t_win **)&param,
			check_and_change(((t_win *)param), 2, NULL, NULL), 0, 0);
	if (keysum == XK_d || keysum == XK_Right)
		put_images_to_win((t_win **)&param,
			check_and_change(((t_win *)param), 1, NULL, NULL), 0, 0);
	return (0);
}
